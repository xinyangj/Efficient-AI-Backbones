...
parser.add_argument('--model', default='pvig_s_32_gelu', type=str, metavar='MODEL',
...
#    pdb.set_trace()
    ...
    resume_epoch = None
    if args.resume:
        #checkpoint_file = sorted(glob.glob(args.resume + '/recovery*.pth.tar'), reverse = True)
        checkpoint_file = sorted(glob.glob(args.resume + '/*.pth.tar'), reverse = True)
        if len(checkpoint_file) > 0:
            resume_epoch = resume_checkpoint(
                model.module, checkpoint_file[0],
    ...
    if args.local_rank == 0:
        ...
        #output_dir = get_outdir(output_base, 'train', exp_nam)
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if loss_scaler is not None:
            ...
            print(batch_idx, 'fooo')
            if args.clip_grad is not None:
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip_grad)
            optimizer.step()
            print(batch_idx, 'foooooo')
